...
func main() {
	...
	r := api.NewRouter(h, cfg.Auth.FrontendURL)
	...
	if cfg.Worker.Enabled {
		...
		jobs := []worker.ScheduledSyncer{
			{Syncer: worker.NewMarketSyncer(gammaClient, marketRepo, cfg.Worker.MaxMarketsPerSync), Interval: cfg.Worker.MarketSyncerInterval},
			{Syncer: worker.NewTradeSyncer(
				dataClient,
				walletRepo,
				marketRepo,
				tokenRepo,
