...
```bash
./scripts/bootstrap_dev.sh    # Start Docker stack, smoke test
```
...
### Database
- Schema managed by GORM AutoMigrate (`DATABASE_AUTO_MIGRATE=true` by default); schema changes are defined in GORM model struct tags (`internal/model/models.go`)
- No hand-written SQL migration files — all DDL is derived from Go structs
...
- **Schema changes:** modify GORM struct tags in `internal/model/models.go`; AutoMigrate applies on startup
